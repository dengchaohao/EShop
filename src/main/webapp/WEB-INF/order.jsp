<%--@elvariable id="defaultAddress" type="cn.edu.ncu.pojo.Address"--%>
<%--
    ...
    <div class="orderCon wrapper clearfix">
        <div class="orderL fl">
            ...
            <div class="addres clearfix">
                <div class="addre fl on">
                    <div class="tit clearfix">
                        <p class="fl">${defaultAddress.contact}
                ...
                <c:if test="${allAddresses!=null}">
                    <c:forEach items="${allAddresses}" var="address" >
                        <div class="addre fl">
                            <div class="tit clearfix">
                                <p class="fl">${address.contact}
                                </p>
                                <p class="fr">
                                    <a href="setDefaultAddress?id=${address.addressId}&defaultAddressId=${defaultAddress.addressId}" class="setDefault">设为默认</a>
                                    <span>|</span>
                                    <a href="#">删除</a>
                                    <span>|</span>
                                    <a href="#" class="edit">编辑</a>
            ...
            <div class="way clearfix">
                <img class="on" src="img/temp/way01.jpg" id="1">
                <img src="img/temp/way02.jpg" id="2">
                <img src="img/temp/way03.jpg" id="3">
                <img src="img/temp/way04.jpg" id="4">
            </div>
            <h3>选择快递</h3>
            <!--------dis---------------->
        ...
        <div class="orderR fr">
            <div class="msg">
                ...
                <!--------ul---------------->
                <%--@elvariable id="cartOrder" type="cn.edu.ncu.pojo.Cart"--%>
                <c:forEach items="${cartOrder.goods}" var="entry">
                    <ul class="clearfix">
                        <li class="fl">
                            <img style="height: 87px;width: 87px" src="${entry.value.value}">
                        </li>
                        <li class="fl">
                            <p>${entry.key.key.name}</p>
                            <p>颜色分类：${entry.key.value}</p>
                            <p>数量：${entry.value.key}</p>
                        </li>
                        <li class="fr">￥${entry.key.key.price*entry.value.key}</li>
                    </ul>
                </c:forEach>
            </div>
            <!--------tips---------------->
            <div class="tips">
                <p><span class="fl">商品金额：</span><span class="fr">￥${cartOrder.totalPrice}</span></p>
                <p><span class="fl">优惠金额：</span><span class="fr">￥0.00</span></p>
                <p><span class="fl">运费：</span><span class="fr">免运费</span></p>
            </div>
            <!--------tips count---------------->
            <div class="count tips">
                <p><span class="fl">合计：</span><span class="fr">￥${cartOrder.totalPrice}</span></p>
            </div>
            <!--<input type="button" name="" value="去支付"> -->
            <a href="#" onclick="myjs()" class="pay">去支付</a>
        </div>
    </div>
</div>
<!--编辑弹框-->
...
<script type="text/javascript" >
    function myjs(){
        var id = $(".addre.on").children(".id").val();
        var payWay = $(".way.clearfix").children(".on").attr("id");
        var deliverWay = $(".dis.clearfix").children(".on").text();
            $.ajax({
                url:'addGoodsOrder',
                data:{
                    id:id,

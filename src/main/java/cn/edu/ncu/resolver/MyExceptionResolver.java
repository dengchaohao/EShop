...
public class MyExceptionResolver implements HandlerExceptionResolver {
    ...
    public ModelAndView resolveException(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object o, Exception e) {
        ...
        if (e instanceof IncorrectCredentialsException || e instanceof UnknownAccountException) { // 密码不正确或用户名不存在
            // 跳转登录页面，重新登录
            mv.setViewName("login");
            mv.addObject("msg", "*账号或密码不正确");
        }else if(e instanceof UnauthorizedException){       // 权限不足 角色不足
            mv.setViewName("redirect:/user/perms/error"); // 重定向时，重新给浏览器发一个请求
